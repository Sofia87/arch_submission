window.gest = (function (window) {
	...
	dispatchGestEvent = function(_gestEvent) {
		...
		eventObj.direction = _gestEvent.direction || null;
		eventObj.up = _gestEvent.up || false;
		eventObj.down = _gestEvent.down || false;
		eventObj.left = _gestEvent.left || false;
		eventObj.right = _gestEvent.right || false;
		eventObj.error = _gestEvent.error || null;
	...
	init = function () {
		...
		if (!!video.canPlayType && !!(canvas.getContext && canvas.getContext('2d')) && !!navigator.getUserMedia) {
			video.width = 300;
			video.height = 225;
